...
public class RemoteGUIExecutorViewController extends GUIExecutorViewController {
	...
	public ExecutorInfo getExecutor(String name, boolean isDefault, boolean isStick2Host, Integer maxSlaveRunning, String path2java, int maxRunning,String watchdogBaseDir, Environment environment, String workingDir ) {
		RemoteExecutorInfo r = new RemoteExecutorInfo(name, isDefault, isStick2Host, maxSlaveRunning, path2java, maxRunning, watchdogBaseDir, environment, this.host.getText(), this.user.getText(), Integer.parseInt(this.port.getText()), !this.disableStrictHostCheck.isSelected(), workingDir);
		r.setPrivKey(this.privateKey.getText());
		return r;
	}
