	...
	public Response serve(String uri, Method method, Map<String, String> headers, Map<String, String> parms, Map<String, String> files) {
		...
		try {
			if(uri.endsWith(JS)) {
				...
				return NanoHTTPD.newChunkedResponse(Response.Status.OK, MIME_JS, in);
			}
			else if(uri.endsWith(CSS)) {
				InputStream in = new FileInputStream(BASE + File.separator + WEB_BASE + File.separator + uri);
				return NanoHTTPD.newChunkedResponse(Response.Status.OK, MIME_CSS, in);
			}
			else if(uri.endsWith(PNG)) {   
				InputStream in = new FileInputStream(BASE + File.separator + WEB_BASE + File.separator + uri);
				return NanoHTTPD.newChunkedResponse(Response.Status.OK, MIME_PNG, in);
			}
		}
		catch(Exception e) {
			return NanoHTTPD.newFixedLengthResponse(Response.Status.NOT_FOUND, MIME_PLAINTEXT, "Resource '"+uri+"' was not found on the server.");
		}
		...
		if(action == null && !parms.containsKey(ACTION)) 
			return NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "No action was set using '"+ACTION+"'!");
		
		// test, if a hash is given
		if(!parms.containsKey(Mailer.HASH) && action != null)
			return NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "Verification id was not found. Do not use old links or manipulte them!");
		...
		if(action != null && !Mailer.verifyLink(uri, parms, hash)) {
			return NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "Verification id not valid. Do not use old links or manipulte them!");
		}
			
		// test, if the link is valid
		if(this.INVALID_URLS.contains(parms.get(Mailer.COUNTER))) {
			return NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "The link you used was only valid one time!");
		}
		...
		if(!parms.containsKey(TASK_ID) && !parms.containsKey(XML_ID) && (action != null && !action.requiresNoID()))
				return NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "No task ID or XML ID was given using '"+TASK_ID+"' or '"+XML_ID+"'!");
		
		if(action == null)
			return NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "Action '"+actionString+"' is not valid!");
		else {
			...
			// test, if a task ID is given
				...
				for(String tid : taskIDs) {
					...
					if(performTask != null) {
						...
						return NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "No task with '"+tid+"' exists!");		
				}
			...
			// we got an XML ID or several
				...
				if(x == null)
					return NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "No xml task with '"+xid+"' exists!");
			...
			if(this.customResponse != null) {
				...
				if(settings.containsKey(RELOAD)) {
					if(settings.get(RELOAD) != null) {
						...
						try { 
							...
							r = NanoHTTPD.newChunkedResponse(Response.Status.BAD_REQUEST, MIME_HTML, new ByteArrayInputStream((addReload + old).getBytes("UTF-8"))); 
						}
						catch(Exception e) {};
					}
				}
				...
				if(allOk) {
					...
					return NanoHTTPD.newFixedLengthResponse(Response.Status.OK, MIME_HTML, addReload + "Action '"+ (noLinkVerification.containsKey(ACTION_NAME) ? noLinkVerification.get(ACTION_NAME) : action.getActionName()) +"' was executed successfully.");
				}
				else
					return NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, message.toString());
			}			
		}
	}
		...
		if(action.isUserinteraceAction()) {		
			...
			if(x != null) {
				...
				// display the formular that can be used to enter the stuff
					...
					this.customResponse = NanoHTTPD.newFixedLengthResponse(Response.Status.OK, MIME_HTML, buf.toString());
					return false;
				}
			}
			return false;
			...
			if(x != null) {
				...
				for(String key : settings.keySet()) {
					...
					// wrong format!
						...
						this.customResponse = NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "Parameter with name '"+key+"' has not the correct type '"+r.getType()+"' ('"+value+"').");
				...
				if(retParams.size() > 0) {
					this.customResponse = NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "Values for the following return parameters are missing: " + StringUtils.join(retParams.keySet(), ", "));
					settings.put(RELOAD, Mailer.getLink(ControlAction.RESOLVE, false, t.getID(), true)); // redicrect after this set
					return false;
				}
			...
			if(x != null)
				...
				this.customResponse = NanoHTTPD.newFixedLengthResponse(Response.Status.BAD_REQUEST, MIME_PLAINTEXT, "No corresponding xml task for that task exists!");			
		}
		return false;
	}
